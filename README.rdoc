...
Getting data on book contributors:
...
Iterating through subjects of the book:

  book.subjects.each do |s|
    subject = s
  end

Open Library book keys are partial URLs, which come in format '/books/{OLID}'. To find a book's keys by its ISBN, LCCN, or OCLC use one of these methods:
