...
In order to save changes to Open Library, you need to log in and get a valid session cookie. All you need is your username and password:

  cookie = client.login('username', 'password')

The cookie is returned as a string.

You will also need to register your bot with the {API Usergroup}[http://openlibrary.org/usergroup/api]. Send a message to the {ol-tech mailing list}[http://mail.archive.org/cgi-bin/mailman/listinfo/ol-tech] list for more information!
